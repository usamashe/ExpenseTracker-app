  ...
  switch (action.type) {
    case "ADD_TRANSACTION":
      return {
        ...
        totalIncome:
          state.totalIncome +
          (action.payload.amount > 0 ? action.payload.amount : 0),
        totalExpense:
          state.totalExpense +
          (action.payload.amount < 0 ? action.payload.amount : 0),
      };
    case "DELETE_TRANSACTION":
      const transactionToDelete = state.transactions.filter(
      ...
      return {
        ...
        totalIncome:
          state.totalIncome -
          (transactionToDelete[0].amount > 0
            ? transactionToDelete[0].amount
            : 0),
        totalExpense:
          state.totalExpense -
          (transactionToDelete[0].amount < 0
    ...
    case "EDIT_TRANSACTION":
      const transactionToEdit = state.transactions.filter(
        (transaction) => transaction.id === action.payload
      );
      ...
      return {
        ...
        totalIncome:
          state.totalIncome -
          (transactionToEdit[0].amount > 0 ? transactionToEdit[0].amount : 0),
        totalExpense:
          state.totalExpense -
          (transactionToEdit[0].amount < 0 ? transactionToEdit[0].amount : 0),
